...
thresh = 58; % <<<< set threshold for object of intrest (Vignesh's memb =< 58)
for set = 1:nSet % split label sets
    ...
    for label = 1:label_count % w/n label set, dilate, crop, erode, save masks
        ...
        %mhaWriter(['test1_', name, sprintf('_label_%d',label), ext], raw, [1,1,1], 'uint8'); % save
        ...
        % purge = zeros(size(raw),'single'); % initialize full size volume
        % mask = purge(mask);% set everything outside the mask to zero
        %croppedMask = mask .* raw;  % update mask with raw
        toc
        tic
        idx = find(croppedMask); % log nonzero voxels 
        [ny,nx,nz] = size(croppedMask); % get crop size
        ...
        croppedMask = croppedMask(ya,xa,za); % crop
        finalMask = rawSmall .* logical(croppedMask); % combine
        toc
        tic
        % threshold
        ...
        %suffix = ['se_' dKr '_label_' label];
        mhaWriter(['test1_', name, sprintf('_label_%d',label), ext], finalMask, [1,1,1], 'uint8'); % save
        toc          
    end
end
